...
/*
*	Casting functions used for casting between class.
*/
	...
	if( strcmp( pszSubType, pszBaseType ) != 0 )
	{
		...
		if( result >= 0 && result < sizeof( szBuffer ) )
		{
			...
			const auto retCode = engine.RegisterObjectMethod(
				pszSubType, szBuffer, asFUNCTION( ( Cast_UpCast<SUB_TYPE, BASE_TYPE> ) ), asCALL_CDECL_OBJFIRST );
		...
		{
			const auto retCode = engine.RegisterObjectMethod(
				pszBaseType, szBuffer, asFUNCTION( ( Cast_DownCast<BASE_TYPE, SUB_TYPE> ) ), asCALL_CDECL_OBJFIRST );
